...
- 内容:
	...
	- 目的とアプローチ:
		...
		- CGI（Common Gateway Interface）を実装し、外部スクリプトの実行および動的レスポンスの生成をサポート
		- RFC規格に基づいたヘッダーバリデーション（検証）および、基本的なHTTP通信プロトコルの実装。
...
- 使用技術: C
- 内容:
	- 概要: サイバーセキュリティ分野の学習の一環で自己複製プログラム（クワイン）を作成。
	- 目的とアプローチ:
		- 外部ファイルI/Oを使用しない自己複製の数学的ロジックを実装。
...
- 内容:
	...
	- 目的とアプローチ:
		- パストレーシングを用いたレンダリングエンジンを構築し、光の相互作用や影の計算を実装。
...
- 使用技術: C
- 内容:
	- 概要: プロセス管理やシグナルと言語処理系の基本を理解するため、Bash互換のコマンドラインインタプリタを共同開発(2名体制)。
	- 目的とアプローチ:
